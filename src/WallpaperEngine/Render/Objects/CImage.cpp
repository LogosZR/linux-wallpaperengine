...
void CImage::setup () {
    ...
    if (this->m_image.model->passthrough) {
		...
		// Historical behavior used to early-return here when all effects
		// were currently invisible. Don't — we want passes built so IPC
		// toggling a user property (e.g. vhs=true) can make them visible
		// at render time without a respawn.
    }
...
void CImage::setupPasses () {
    ...
    // enabled effect set. This is called per-frame from render() so
    // toggling an effect on/off via IPC immediately re-wires the chain.
