...
void GLFWWindowOutput::updateRender () const {
    ...
    // runtime resizes (reposition IPC, Kuro's tweak drawer, host window
    // resize) re-stretch the scene across the new viewport instead of
    // cropping the original render at the old size.
    this->m_fullWidth = this->m_driver.getFramebufferSize ().x;
    this->m_fullHeight = this->m_driver.getFramebufferSize ().y;
